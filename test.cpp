...
int main() {
    ...
    currentUser = name;
        ...
        while (!validPin && invalidPinAttempts < 5 && wrongPinAttempts < 3) {
    ...
    cin.ignore();
            ...
            if (pin.length() != 4 || !all_of(pin.begin(), pin.end(), ::isdigit)) {
                invalidPinAttempts++;
                cout << "Invalid PIN format. PIN must be exactly 4 digits.\n";
                cout << "Attempts remaining: " << (5 - invalidPinAttempts) << "\n";
                if (invalidPinAttempts >= 5) {
                    throw InputException("Too many invalid PIN format attempts");
                }
                continue;
            }
    ...
    filename = name + "_" + pin + ".txt";
            ...
            // If file exists, check if it's a valid save file
            if (checkFile.good()) {
                checkFile.close();
                validPin = true;
            } else {
                ...
                cout << "Attempts remaining: " << (3 - wrongPinAttempts) << "\n";
                if (wrongPinAttempts >= 3) {
                    throw InputException("Too many wrong PIN attempts");
                }
            }
        }
    ...
    bool fileExists = load_game(filename, level, ecopoints, funds, health, hunger,
        ...
        getline(cin, cityName);
            if (cityName.empty()) {
                throw InputException("City name cannot be empty");
            }
    }
