...
public:
    explicit FileException(const string& message) : runtime_error(message) {}
};

class ResourceException : public runtime_error {
public:
    explicit ResourceException(const string& message) : runtime_error(message) {}
};

class InputException : public runtime_error {
public:
    explicit InputException(const string& message) : runtime_error(message) {}
};
...
const int CYCLE_COST = 500;
const int CAR_COST = 2000;
const int ELECTRIC_CAR_COST = 5000;
...
int daysBetween(const string& start, const string& end) {
    try {
        ...
        tm tm1 = {};
        tm1.tm_year = startYear - 1900;
        tm1.tm_mon = startMonth - 1;
        tm1.tm_mday = startDay;
        
        tm tm2 = {};
        tm2.tm_year = endYear - 1900;
        tm2.tm_mon = endMonth - 1;
        tm2.tm_mday = endDay;
    ...
    time_t t2 = mktime(&tm2);
        ...
        cerr << "Error calculating days between dates: " << e.what() << endl;
        return 0;
    }
}
...
public:
    Building(int& f, int& e, int& h, int& hu, int& l, int& lp, int& p, const string& cn, int up=1)
        : funds(f), ecoPoints(e), health(h), hunger(hu), level(l), levelPoints(lp), pollutionLevel(p), cityName(cn) {
        upgrade.level = up;
    }
...
public:
    House(int& f, int& e, int& h, int& hu, int& l, int& lp, int& p, const string& cn, int up=1)
        : Building(f, e, h, hu, l, lp, p, cn, up) {}
...
private:
    ...
    public:
        MathProblemManager() : rng(static_cast<unsigned>(time(nullptr))) {
            initializeProblems();
        }
        
        // Public interface to get problem details
        string getProblemQuestion(int maxDifficulty) {
            MathProblem problem = getRandomProblem(maxDifficulty);
            return problem.question;
        }
        
        int getProblemAnswer(int maxDifficulty) {
            MathProblem problem = getRandomProblem(maxDifficulty);
            return problem.answer;
        }
        
        int getProblemDifficulty(int maxDifficulty) {
            MathProblem problem = getRandomProblem(maxDifficulty);
            return problem.difficulty;
        }
        
        bool checkAnswer(int maxDifficulty, int userAnswer) {
            MathProblem problem = getRandomProblem(maxDifficulty);
            return problem.answer == userAnswer;
        }
        
        int calculateReward(int maxDifficulty, int baseReward) {
            MathProblem problem = getRandomProblem(maxDifficulty);
            return (baseReward * problem.difficulty) / 2;
        }
        
        int calculateConsolationReward(int baseReward) {
            return baseReward / 4;
        }
        
    private:
        MathProblem getRandomProblem(int maxDifficulty) {
    ...
    MathProblemManager problemManager;
    
    bool solveMathProblem() {
        string question = problemManager.getProblemQuestion(upgrade.level);
        ...
        if (problemManager.checkAnswer(upgrade.level, answer)) {
            int baseReward = OFFICE_WORK_REWARD / 2;
            int reward = problemManager.calculateReward(upgrade.level, baseReward);
            cout << "✅ Correct! You earned " << reward << " funds!\n";
            funds += reward;
            return true;
        } else {
            int consolationReward = problemManager.calculateConsolationReward(OFFICE_WORK_REWARD);
            cout << "❌ Wrong! The correct answer was: " << problemManager.getProblemAnswer(upgrade.level) << "\n";
...
public:
    ...
    void workSession() {
        ...
        int hungerReduction = 8 * upgrade.level;
...
void generateNews(const string& category, const string& title, const string& content, bool isImportant) {
    ...
    if (!out.is_open()) {
            throw FileException("Could not open news_feed.txt for writing");
    }
    ...
    this_thread::sleep_for(chrono::seconds(2));
        ...
        cerr << "Error generating news: " << e.what() << endl;
    } catch (const exception& e) {
        cerr << "Unexpected error generating news: " << e.what() << endl;
    }
}
...
void showNewsFeed() {
    ...
    while (true) {
        ...
        cerr << "Error displaying news feed: " << e.what() << endl;
        cout << "Press Enter to continue...";
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
        cin.get();
    } catch (const exception& e) {
        cerr << "Unexpected error displaying news feed: " << e.what() << endl;
        cout << "Press Enter to continue...";
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
        cin.get();
    }
...
void triggerRandomEvent(int& funds, int& health, int& ecopoints, int& pollutionlevel) {
    ...
    if (dist(rng) == 1) {
        ...
        cerr << "Error triggering random event: " << e.what() << endl;
    }
}
...
void updateHungerAndHealth(int& health, int& hunger, time_t& lastUpdateTime) {
    ...
    if (timeDiff >= 60) { // Update every minute
        ...
        cerr << "Error updating hunger and health: " << e.what() << endl;
    }
}
...
bool load_game(string filename, int& level, int& ecopoints, int& funds, int& health, int& hunger,
    ...
    try {
        ...
        health = MAX_HEALTH;
        hunger = MAX_HUNGER;
        houseUp = hospitalUp = officeUp = restaurantUp = schoolUp = bankUp = casinoUp = envUp = recUp = gardenUp = 1;
        vehicle = 0;
        pollutionlevel = 50;
...
void save_game(string filename, int level, int ecopoints, int funds, int health, int hunger,
    ...
    ofstream file(filename);
        if (!file.is_open()) {
            throw FileException("Could not open save file for writing: " + filename);
        }
        ...
        file << "daysWithZeroPollution: " << daysWithZeroPollution << endl;
            ...
            if (file.fail()) {
                throw FileException("Error writing to save file: " + filename);
            }
        } catch (const exception& e) {
        file.close();
            throw FileException("Error during save file write: " + string(e.what()));
        }
        ...
        cerr << "Unexpected error saving game: " << e.what() << endl;
        throw; // Re-throw to handle in main
    }
}
    ...
    int &houseUp, int &hospitalUp, int &officeUp, int &restaurantUp, int &schoolUp, int &bankUp, int &casinoUp, int &envUp, int &recUp, int &gardenUp, const string& cityName, int &daysWithZeroPollution)
{
    ...
    int initialFunds = funds;
        static int lastMenuVisit = 0;
    
        while (true) {
        if (health <= 0 || hunger <= 0) {
                throw ResourceException("Game over: Health or hunger reached zero");
        }
        ...
        lastMenuVisit++;
            ...
            if (pollutionLevel == 0) {
                ...
                if (level >= WIN_REQUIRED_LEVEL && daysWithZeroPollution >= WIN_REQUIRED_DAYS) {
                        throw ResourceException("Victory achieved: Eco-city goals met!");
                }
            } else {
                daysWithZeroPollution = 0;
            }
        ...
        generateRandomNews(level, ecopoints, pollutionLevel, cityName);
                ...
                cerr << "Error updating game state: " << e.what() << endl;
            }
        ...
        int choice;
            if (!(cin >> choice)) {
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                throw InputException("Invalid menu choice");
            }
        ...
        int netFunds = funds - initialFunds;
            ...
            case 3:
                ...
                break;
            case 4:
                        handleTransportChoice(vehicle, level, funds);
                        break;
                    case 5:
                ...
                cerr << "Error: " << e.what() << endl;
                throw; // Re-throw to handle game over/victory conditions
            } catch (const exception& e) {
        ...
        cout << "Eco Points: " << ecopoints << "\n";
        cout << "Funds: " << funds << "\n";
        if (e.what() == string("Victory achieved: Eco-city goals met!")) {
...
void handleTransportChoice(int& vehicle, int level, int& funds) {
    try {
            ...
            default: throw InputException("Invalid vehicle type");
                }
                cout << "\n\nAvailable vehicles:\n";
                cout << "0. Walk (Free)\n";
        ...
        if (!(cin >> choice)) {
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
            throw InputException("Invalid transport choice");
        }
        ...
        switch (choice) {
            ...
            case 1:
                    if (level < CYCLE_REQUIRED_LEVEL) {
                    throw ResourceException("Need level " + to_string(CYCLE_REQUIRED_LEVEL) + " to buy a bicycle");
                }
                if (!hasBicycle) {
                    if (funds < CYCLE_COST) {
                        throw ResourceException("Not enough funds to buy bicycle");
                    }
                            funds -= CYCLE_COST;
                            hasBicycle = true;
                            vehicle = 1;
            ...
            case 2:
                    if (level < CAR_REQUIRED_LEVEL) {
                    throw ResourceException("Need level " + to_string(CAR_REQUIRED_LEVEL) + " to buy a regular car");
                }
                if (!hasCar) {
                    if (funds < CAR_COST) {
                        throw ResourceException("Not enough funds to buy regular car");
                    }
                            funds -= CAR_COST;
                            hasCar = true;
                            vehicle = 2;
            ...
            case 3:
                    if (level < ELECTRIC_CAR_REQUIRED_LEVEL) {
                    throw ResourceException("Need level " + to_string(ELECTRIC_CAR_REQUIRED_LEVEL) + " to buy an electric car");
                }
                if (!hasElectricCar) {
                    if (funds < ELECTRIC_CAR_COST) {
                        throw ResourceException("Not enough funds to buy electric car");
                    }
...
int main() {
    try {
        ...
        int level = 1;
        int ecopoints = 0;
        int funds = 1000;  // Starting funds
        int health = MAX_HEALTH;
        int hunger = MAX_HUNGER;
        int levelPoints = 0;
        int pollutionLevel = 50;  // Starting pollution level
        int vehicle = 0;  // Start with walking
        ...
        // Login/Registration
        string username, pin;
        cout << "Welcome to Eco City!\n";
        cout << "Enter username: ";
        getline(cin, username);
        cout << "Enter PIN: ";
        getline(cin, pin);

        string filename = username + "_" + pin + ".txt";

        // Try to load existing game
        if (!load_game(filename, level, ecopoints, funds, health, hunger,
            houseUp, hospitalUp, officeUp, restaurantUp, schoolUp, bankUp, casinoUp, envUp, recUp, gardenUp,
            vehicle, pollutionLevel, lastSaveTime, levelPoints,
            hasLoan, loanAmount, lenderName, loanStartDate, cityName, daysWithZeroPollution)) {
        ...
        try {
            menu(funds, vehicle, level, lastUpdateTime, ecopoints, health, hunger, levelPoints, pollutionLevel,
                username, hasLoan, loanAmount, lenderName, loanStartDate,
                houseUp, hospitalUp, officeUp, restaurantUp, schoolUp, bankUp, casinoUp, envUp, recUp, gardenUp,
                cityName, daysWithZeroPollution);
        ...
        cerr << "Resource error: " << e.what() << endl;
        return 1;
    } catch (const exception& e) {
        cerr << "Unexpected error: " << e.what() << endl;
        return 1;
